...
;(function () {
	...
	BX.Vasoft.Likeit.init = function () {
		...
		if (ids.length > 0) {
			BX.ajax.runAction('vasoft:likeit.Likes.list', {data: {ids: ids}})
				.then(BX.Vasoft.Likeit.onList);
		}
	}
	...
	BX.Vasoft.Likeit.onClick = function () {
		...
		if (id > 0) {
			BX.ajax.runAction('vasoft:likeit.Likes.like', {data: {id: id}})
				.then(BX.Vasoft.Likeit.onLike);
		}
		return false;
	}
